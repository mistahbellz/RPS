...
//compare the scores to decide final winner(first to score 5)4

 function compare(p1, cpu) {
 if (p1.score === 5) {gameOn = false;  refMsg.textContent = `${p1.name} is the Winner with a Score of ${p1.score}`;} 
 else if (cpu.score === 5) {gameOn = false;  refMsg.textContent = `${cpu.name} is the Winner with a Score of ${cpu.score}`;}
}
